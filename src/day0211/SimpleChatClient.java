...
public class SimpleChatClient extends JFrame implements ActionListener, Runnable {
	...
	public SimpleChatClient(String ip) {
		...
		try {
			connectToServer(ip);
//			readMsg();
			//Thread클래스 생성 후 이 클래스와 has a 관계 설정
			Thread t = new Thread(this);
			//메시지를 무한루프로 읽어들이는 코드를 Thread로 실행한다.
			t.start();// start() => run() 호출
...
//		client = new Socket("211.63.89."+ip, 50000);//서버에 연결
		client = new Socket("211.63.89."+ip, 25000);//서버에 연결
...
//	private void readMsg() throws IOException {
	@Override
	public void run()/*thows IOException*/ {
		try {
		while(true) {
			...
			ie.printStackTrace();
			JOptionPane.showMessageDialog(this, "접속자 접속을 종료했습니다.");
		}
	}//readMsg
	...
	private void sendMsg() throws IOException{
		...
		sbMsg.append("최연이 클라이언트: ").append(msg);
